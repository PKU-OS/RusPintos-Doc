...
In the toolset, we've provided a build tool. Run `cargo h build` or `cargo bd --help` for the usage (the alias of build tool is `cargo bd`):
...
To manually build user programs, you can run `make` under `./`. All of the user programs are written in C and locate under `./user/`. If you want to write your own user programs, you can add `*.c` files under `./user/userprogs/` or `./user/vm/`. If you need to add a new sub-directory, like `./user/yours/`, you'll need to add `user/yours` to the `SRC_DIRS` in `./user/utests.mk`, otherwise your new programs will not be added into disk image and your OS will not find it.
...
The toolset also provides a tool to test your OS for the test cases that are included in the grading process. Run `cargo h test` or `cargo tt --help` for the usage (the alias of test tool is `cargo tt`):
...
You can use the shown command under `./` to test the `args-many` case. In this way, you can see the entire log info of your OS. In default way, the test tool will only report the last 10 lines. You can also use `-v` for the entire output, however, the grading mode and gdb mode will not function under verbose mode.

## Bookmark

In the test section, we said that the test tool can receive bookmarks in `./tool/bookmarks/`, and you can build your own bookmarks for convenience. The bookmark tool is provided to let you build your bookmarks. Run `cargo h book` or `cargo bk --help` to see the usage (the alias of bookmark tool is `cargo bk`):
...
Here is an example, which creates a bookmark named `mybk.toml` and including `args-none, args-many` test cases:
